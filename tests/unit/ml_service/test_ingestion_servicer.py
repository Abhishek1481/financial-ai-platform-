...
async def _extract(server_port: int, uri: str, doc_type: int) -> ingestion_pb2.ExtractDocumentResponse:
...
async def test_extracts_txt_file_end_to_end(server_port: int, tmp_path: Path):
    ...
    response = await _extract(server_port, target.as_uri(), common_pb2.DOCUMENT_TYPE_TXT)

    assert response.raw_text == "Q1 revenue grew 12%."
    assert response.page_count == 1


async def test_extracts_sec_filing_and_infers_filing_type(server_port: int, tmp_path: Path):
    ...
    response = await _extract(server_port, target.as_uri(), common_pb2.DOCUMENT_TYPE_SEC_FILING)
...
async def test_malformed_document_returns_invalid_argument(server_port: int, tmp_path: Path):
